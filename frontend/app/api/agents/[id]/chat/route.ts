...
async function agentBondingCurveBuy(
  ...
  try {
    ...
    // V5 inline-migration, env-gated. When NEXT_PUBLIC_INLINE_MIGRATION_ENABLED
    // is true and we can resolve the token's CoinMetadata, call the 11-arg
    // inline entry so a graduating buy creates the Cetus pool in the same
    // tx. Falls back to the 6-arg dump-path buy otherwise (old cron still
    // catches those). Same pattern as the main buy page.
    const inlineEnabled = process.env.NEXT_PUBLIC_INLINE_MIGRATION_ENABLED === 'true'
    let inlineBuilt = false
    if (inlineEnabled) {
      try {
        const tokenMeta = await suiClient.getCoinMetadata({ coinType })
        if (tokenMeta?.id) {
          tx.moveCall({
        ...
        // Fall through to the non-inline path below.
      }
    }
