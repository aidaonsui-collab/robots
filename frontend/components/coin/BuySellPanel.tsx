...
// NOTE: this panel is currently not wired into any page (slug page has its
// own inline trade panel). It targets the legacy `buy(pool, payment, minOut)`
// signature which only exists on the legacy v7 upgrade chain. v11 replaced
// it with `buy_exact_in_with_lock(configuration, ...)`. If this panel is
// re-adopted it needs a rewrite to handle v11 pools.
import { MOONBAGS_CONTRACT_LEGACY, QUOTE_COIN, CURVE_CONFIG } from '@/lib/contracts'
...
export default function BuySellPanel({ token }: BuySellPanelProps) {
  ...
  const executeBuy = async () => {
    ...
    const [memeCoins] = tx.moveCall({
      target: `${MOONBAGS_CONTRACT_LEGACY.packageId}::${MOONBAGS_CONTRACT_LEGACY.module}::buy`,
  ...
  const executeSell = async () => {
    ...
    const [suiCoins] = tx.moveCall({
      target: `${MOONBAGS_CONTRACT_LEGACY.packageId}::${MOONBAGS_CONTRACT_LEGACY.module}::sell`,
      typeArguments: [tokenType, QUOTE_COIN],
      arguments: [
        tx.object(poolId),
        tokenCoin,
