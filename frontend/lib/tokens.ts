...
export async function fetchPoolToken(poolIdOrCoinType: string): Promise<PoolToken | null> {
  try {
    ...
    const allCreatedEvents = creationEventResponses.flatMap((j: any) => j?.result?.data || [])
    const creationEvent = allCreatedEvents.find(
      (e: any) => e.parsedJson?.pool_id === poolId
    )
...
const HIDDEN_TOKENS = new Set([
  ...
  '0x89a87e33c337f5093a78bb1e9451a62d9b29c791c8fa639ba032f398d1a513cc::sword::SWORD',                   // SWORD
